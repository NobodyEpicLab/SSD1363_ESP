...
static esp_err_t write_fullscreen_pattern(uint8_t pattern)
{
	...
	esp_err_t err = ssd1363_api_set_window(
		0,
...
void app_main(void)
{
	...
	printf("Smoke-test active area: %dx%d, multiplex=0x%02X, offset=0x%02X, column_offset=%u, remap=%02X %02X\n",
	...
	printf("Address 0x%02X acknowledged, sending smoke-test init sequence\n", SSD1363_I2C_ADDRESS);

	err = ssd1363_api_init_smoke_test_display();
	if (err != ESP_OK) {
		printf("Smoke-test init sequence failed: %s\n", esp_err_to_name(err));
		return;
	}

	printf("Init sequence sent, cycling fill and stripe patterns\n");
